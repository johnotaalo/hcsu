...
class Form25{
	...
	public function getData($case_no, $document){
		...
		if($form_data->client->clientdata->MARITAL_STATUS == "Single"){
			$spouse_name = ($form_data->client->clientdata->parent) ? format_other_names($form_data->client->clientdata->parent->OTHER_NAMES) . " " . strtoupper($form_data->client->clientdata->parent->LAST_NAME) . "({$form_data->client->clientdata->parent->relationship})" : "N/A";
		}else{
			if($spouseDetails){
