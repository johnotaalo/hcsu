...
class SendOrganizationData implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if (!\Storage::exists($filename) || !\Storage::exists($oldDataFileName)) {
            ...
            $data = $queryBuilder->whereRaw('CONVERT(agency USING utf8mb4) LIKE ?', ["{$this->searchBy}%"])
        ...
        $response = Mail::to($this->recepients)->send(new \App\Mail\DataDumpSent($filename, $oldDataFileName, $this->group));
        \Log::debug($response);
        // try{
