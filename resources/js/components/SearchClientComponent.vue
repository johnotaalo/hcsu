		...
		<div v-else>
			<div v-if="clientType == 'agency'">
				<div v-if="applier != 'fp'">
					...
					<v-select label="AGENCYNAME" :filterable="false" :options="agencies" @search="onAgencySearch" v-model="value.host_country_id">
				...
				<div v-else>
					<p>VAT Application for: </p>
				</div>
			</div>
			<div v-if="clientType == 'staff-member'">
				<label class="control-label">Please type to search for a staff member</label>
				<v-select label = "LAST_NAME" :filterable="false" :options="staffMembers" @search="onStaffSearch" v-model="value.host_country_id">
					<template slot="no-options">
